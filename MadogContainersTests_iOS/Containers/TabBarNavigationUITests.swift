...
class TabBarNavigationUITests: ContainersKIFTestCase {
    ...
    func testRenderInitialUI() throws {
        try container = renderUIAndAssert(tokens: "vc1", "vc2")
    ...
    func testBackToRoot() throws {
        ...
        try navigateForwardAndAssert(token: "vc4")
        waitForAbsenceOfLabel(token: "vc3")
    ...
    func testNavigateForwardAndBack_multitab() throws {
        container = try renderUIAndAssert(tokens: "vc1", "vc2")
        try navigateForwardAndAssert(token: "vc3")
    ...
    func testOpenMultiNavigationModal() throws {
        container = try renderUIAndWait(identifier: .basic(), tokenData: .single("vc1"))
        waitForLabel(token: "vc1")
        XCTAssertNotNil(container)
        ...
        try modalContainer.forwardBack?.navigateForward(token: "vc4", animated: true)
        waitForTitle(token: "vc4")
        waitForLabel(token: "vc4")

        try modalContainer.forwardBack?.navigateForward(token: "vc5", animated: true)
        try modalContainer.forwardBack?.navigateForward(token: "vc6", animated: true)
        ...
        try modalContainer.forwardBack?.navigateForward(token: "vc7", animated: true)
        waitForTitle(token: "vc3") // "Back" shows "vc3"
        waitForTitle(token: "vc7")
        waitForLabel(token: "vc7")

        try modalContainer.forwardBack?.navigateForward(token: "vc8", animated: true)
        try modalContainer.forwardBack?.navigateForward(token: "vc9", animated: true)
    ...
    private func renderUIAndAssert(tokens: String ...) throws -> AnyContainer<String> {
        let container = try renderUIAndWait(identifier: .tabBarNavigation(), tokenData: .multi(tokens))
