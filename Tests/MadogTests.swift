...
class MadogTests: XCTestCase {
    ...
    func testMadogKeepsStrongReferenceToCurrentContext() {
        ...
        weak var context1 = madog.renderUI(identifier: .navigation(), tokenData: SingleUITokenData("match"), in: window)
        XCTAssertNotNil(context1)

        weak var context2 = madog.renderUI(identifier: .navigation(), tokenData: SingleUITokenData("match"), in: window)
